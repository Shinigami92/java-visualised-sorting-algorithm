...
/**
 * Class which wrapps a Service and delegates the calls to this service in the
 * UI-Thread. It helps you to access the values of the Services from outside of
 * the UI-Thread.
 * 
 * @author sialcasa
 */
public class ServiceWrapper implements Worker {
	...
	/**
	 * Calls the {@link Service#start()} method of the given service and blocks
	 * the caller thread until the state RUNNING, FAILED, or CANCELLED (was
	 * RUNNING befor) is reached.
	 * 
	 * @param timeout
	...
	/**
	 * Calls the {@link Service#restart()} method of the given service and
	 * blocks the caller thread until the state RUNNING, FAILED, or CANCELLED
	 * (was RUNNING befor) is reached.
	 * 
	 * @param timeout
	 *            maximum duration of the call
	 * 
	...
	private void callAndWaitService(long timeout, Runnable serviceCall)
	...
	/**
	 * Starts the {@link Service} and blocks the caller thread until it reached
	 * the @targetValue of the {@link ObservableValue} which is taken by
	 * the @observableGetter. This could be for example the
	 * {@link Service#valueProperty()}.
	 * 
	 * @param observableGetter
	 *            which is the value to be testet (e.g.
	 *            {@link Service#valueProperty()})
	 * @param targetValue
	 *            which is the value to be reached (e.g. State
	 *            {@link State.SUCCEEDED})
	 * @param timeoutInSeconds
	 *            that fails the execution if the @targetValue could not reached
	 *            on @observableGetter in time
	 * @return
	 */
	public void startAndWaitForValue(ObservableGetter observableGetter, Object targetValue, long timeoutInSeconds)
			throws InterruptedException, ExecutionException, TimeoutException {
		callService(observableGetter, targetValue, timeoutInSeconds, () -> service.start());
	}

	/**
	 * Restarts the {@link Service} and blocks the caller thread until it
	 * reached the @targetValue of the {@link ObservableValue} which is taken by
	 * the @observableGetter. This could be for example the
	 * {@link Service#valueProperty()}.
	 * 
	 * @param observableGetter
	 ...
	 *            on @observableGetter in time
	 */
	public void restartAndWaitForValue(ObservableGetter observableGetter, Object targetValue, long timeoutInSeconds)
			throws InterruptedException, ExecutionException, TimeoutException {
		callService(observableGetter, targetValue, timeoutInSeconds, () -> service.restart());
	}

	private void callService(ObservableGetter observableGetter, Object targetValue, long timeoutInSeconds,
			Runnable serviceCall) throws InterruptedException, ExecutionException, TimeoutException {
